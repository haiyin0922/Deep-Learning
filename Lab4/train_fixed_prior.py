...
from utils import init_weights, kl_criterion, plot_pred, finn_eval_seq, pred, plot1, plot2
...
def train(x, cond, modules, optimizer, kl_anneal, args):
    ...
    for i in range(1, args.n_past + args.n_future):
        ...
        g_t = modules['frame_predictor'](torch.cat([cond[i], h, z_t], 1))
...
class kl_annealing():
    def __init__(self, args):
        ...
        self.step = 1 / ((args.niter/args.kl_anneal_cycle)*args.kl_anneal_ratio)
        self.step_mono = 1 / (args.niter/args.kl_anneal_cycle)
        self.iter = args.epoch_size
        self.cnt = 0
    
    def update(self):
        # raise NotImplementedError
        if self.cyclical == True:
            self.cnt += 1
            if self.beta < 1:
                self.beta += self.step
                ...
                self.beta = self.step
                self.cnt = 0
        elif self.cyclical == False and self.beta < 1:
            self.beta += self.step_mono
...
def main():
    ...
    best_val_psnr = 0
    args.tfr_decay_step = 1 / (args.niter-args.tfr_start_decay_epoch)
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        for _ in range(args.epoch_size):
            ...
            loss, mse, kld = train(seq.to(device), cond.to(device), modules, optimizer, kl_anneal, args)
            epoch_loss += loss
            epoch_mse += mse
            epoch_kld += kld
        ...
        if epoch % 5 == 0:
            ...
            for _ in range(len(validate_data) // args.batch_size):
                try:
                    ...
                    validate_seq, validate_cond = next(validate_iterator)
                with torch.no_grad():
                    pred_seq = pred(validate_seq.to(device), validate_cond.to(device), modules, args, device)
                validate_seq = validate_seq.transpose_(0, 1)
            ...
            tfr_rec.append(args.tfr)
            kl_weight_rec.append(kl_anneal.get_beta())
            loss_rec.append(epoch_loss/args.epoch_size)
            psnr_rec.append(ave_psnr)
        ...
        if epoch % 20 == 0:
            ...
            with torch.no_grad():
                plot_pred(validate_seq.to(device), validate_cond.to(device), modules, epoch, args, 'validate')

    plot1(epoch_rec, tfr_rec, kl_weight_rec, loss_rec)
    plot2(epoch_rec, tfr_rec, kl_weight_rec, psnr_rec)
